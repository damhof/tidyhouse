...
import { switchUser, getAllUsers } from '@/lib/actions';
...
export function UserGate() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-warm-50 dark:bg-warm-950 p-4">
      <div className="bg-white dark:bg-warm-800 rounded-2xl shadow-xl p-8 max-w-sm w-full text-center border border-warm-200 dark:border-warm-700">
        ...
        <div className="flex gap-4 justify-center">
          {users.map(u => (
            <button key={u.id} onClick={() => handleSelect(u.id)}
              className="flex flex-col items-center gap-2 p-6 rounded-2xl bg-warm-50 dark:bg-warm-900 hover:bg-sage-50 dark:hover:bg-sage-900/30 hover:ring-2 ring-sage-400 transition-all duration-200 min-w-[120px] active:scale-95">
              <span className="text-5xl">{u.avatarEmoji}</span>
