...
export function BottomNav() {
  ...
  return (
    <nav aria-label="Main navigation" className="fixed bottom-0 left-0 right-0 z-50 bg-white/90 dark:bg-warm-900/90 backdrop-blur-lg border-t border-warm-200 dark:border-warm-800 md:hidden" style={{ paddingBottom: 'env(safe-area-inset-bottom)' }}>
...
export function SideRail() {
  ...
  return (
    <nav aria-label="Main navigation" className="hidden md:flex flex-col w-20 lg:w-56 h-screen fixed left-0 top-0 bg-white dark:bg-warm-900 border-r border-warm-200 dark:border-warm-800 py-6 px-2 lg:px-4">
      <div className="text-center lg:text-left mb-8 px-2">
        ...
        <span className="text-2xl lg:hidden" aria-label="TidyHouse">🏡</span>
      </div>
      <div className="flex flex-col gap-1 flex-1">
        {navItems.map(renderNavItem)}
      </div>
      <div className="flex flex-col gap-1 border-t border-warm-200 dark:border-warm-800 pt-4 mt-4" role="group" aria-label="Secondary navigation">
