...
export default async function ProjectDetailPage({ params }: { params: Promise<{ projectId: string }> }) {
  ...
  if (!project) return (
    <div className="text-center py-20">
      ...
      <h2 className="text-xl font-bold text-warm-800 dark:text-warm-100 mb-2">Project not found</h2>
      <Link href="/projects" className="text-sage-600 dark:text-sage-400 hover:underline text-sm">← Back to projects</Link>
    </div>
  );
  ...
  return (
    <div className="space-y-6">
      <Link href="/projects" className="text-sage-600 dark:text-sage-400 hover:underline text-sm inline-flex items-center gap-1">
        ← Back to projects
      </Link>
      ...
      <div className="bg-white dark:bg-warm-800 rounded-2xl p-6 shadow-sm border border-warm-200 dark:border-warm-700">
        <div className="flex items-center justify-between mb-4">
          ...
          {totalTasks > 0 && (
            <span className="text-sm text-warm-500 font-medium">{doneCount}/{totalTasks} done</span>
          )}
        </div>
        {totalTasks > 0 && (
          <div className="h-2 bg-warm-200 dark:bg-warm-600 rounded-full overflow-hidden mb-4">
            <div className="h-full bg-sage-500 rounded-full transition-all duration-500 ease-out"
          ...
          {activity.map(a => (
            <div key={a.id} className="flex items-start gap-2.5 text-sm py-1">
              ...
              <div className="flex-1">
                <p className="text-warm-600 dark:text-warm-300">{a.details}</p>
                <p className="text-xs text-warm-400">{new Date(a.createdAt).toLocaleString()}</p>
              </div>
            </div>
          ))}
          {activity.length === 0 && (
            <p className="text-sm text-warm-400 text-center py-6">No activity yet</p>
          )}
        </div>
      </div>
    </div>
